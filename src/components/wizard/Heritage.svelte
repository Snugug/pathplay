<script>
  ...
  setContext('filter', {
    source,
    results,
    placeholder: 'Filter heritages...',
  });

  $: {
    const heritages = $choices.ancestry?.heritages || [];
    // Get ancestries
    filterCollection('data', [{ key: 'url', value: heritages, operator: 'in' }])
      .then((her) => {
        if (her) {
          return her
   ...
   */
  function select() {
    choices.set(Object.assign($choices, { heritage: preview }));
    character.set(
