...
const cpus = os.cpus().length > 0 ? Math.min(os.cpus().length, 5) : 1;
...
(async () => {
  try {
    ...
    const scrapers = [
      ...
      .reduce((acc, cur, i, a) => {
        const chunk = Math.ceil(a.length / cpus);
    ...
    const overall = progress.create(scrapers.flat().length, 0, {
      title: 'Scrapers',
    });

    for (const scraper of scrapers.flat()) {
      progress.log(colors.cyan(`Starting ${scraper.title}`));
    ...
    //           path.join(__dirname, '/utils/workers.js'),
    //           {
    //             workerData: s,
    //           },
    //         );
