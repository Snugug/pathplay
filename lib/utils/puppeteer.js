 ...
 */
export async function setupPage(browser, url) {
  const page = await browser.newPage();
  ...
  const items = await Promise.all(
    (
      await page.evaluate(async () => {
        /* global _parseSharedInformation, _parseClasses, _parseAncestry, _parseBackgrounds */
        const items = [
          ...document
      ...
      .map(async (item) => {
        ...
        if (item.type === 'background') {
          await postprocessBackground(item, setupPage, browser);
        }
