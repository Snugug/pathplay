 ...
 * @param {Object} results - The object to store the results in
 * @return {Promise<Object>} - An object containing the parsed class data
 */
async function _parseClasses(item, results) {
  ...
  const featureBody = featureNodes.slice(featuresStart);
  const featureHeaders = featureBody.filter((a) => a.tagName === 'H3');

  const features = _parseFeatures(featureHeaders, featureBody).filter(
    (f) => !basic.includes(f.title),
  );
