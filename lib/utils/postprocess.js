...
export async function postprocessBackground(background) {
  const { processed } = await import('./structured/backgrounds.js');

  const { skills, feats, lore } = processed.find(
 ...
 * @param {String} type
 */
export async function genericPostprocess(item, type) {
  const { processed } = await import(`./structured/${type}.js`);
 ...
 * @return {Object}
 */
export async function postprocessArmor(a) {
  const { armor } = await import('./structured/armor.js');
  const meta = armor.find((b) => b.url === a.url);
 ...
 * @return {Object}
 */
export async function postprocess(a) {
  const { armor } = await import('./structured/armor.js');
  const meta = armor.find((b) => b.url === a.url);
